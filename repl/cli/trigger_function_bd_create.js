// trigger_function_bd_create.js (repl/cli) 190116_1800
...
module.exports = async (db, tableName, columnNames) => {
  ...
  const prefixed_cols_ = columnNames.map(c_ => `OLD.${c_}`)
  const vals_ = `${prefixed_cols_.join(", ")}, '${REPL_OP_DELETE}', current_timestamp`


  return trigger_function_create(db, {
    name: `${REPL_FUNC_BD_PFX}${tableName}`,
    replace: true,
    args: null,
